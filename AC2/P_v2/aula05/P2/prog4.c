...
int main(void)
{
    ...
    TRISC = TRISC & 0xBFFF;
    // initialize ports
    unsigned int counter = 0,i=0,K=0;
    char bcd_counter;
    while (1)
    {
        i = 0;
        do
        {
            bcd_counter = toBCD(counter);
            send2displays(bcd_counter);
            delay(10); // wait 10 ms (1/100Hz)
        } while (++i < K);
        if ((PORTB & 0x0001) == 1){
            counter++;      // increment counter (mod 60)
            K = 50;
        } else {
            counter--;      // decrement counter (mod 60)
            K = 20;
        }
