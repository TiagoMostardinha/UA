...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (displayFlag == 0)
        {
            LATD = (LATD | 0x0040) & 0xFFDF; // enable display low (RD5) and disable display high (RD6)
            LATB = (LATB & 0x80FF) | ((unsigned int)(value) << 8); // send to display
        } else {
            LATD = (LATD | 0x0020) & 0xFFBF; // enable display high (RD6) and disable display low (RD5)
            LATB = (LATB & 0x80FF) | ((unsigned int)(value) << 8); // send to display
        }
