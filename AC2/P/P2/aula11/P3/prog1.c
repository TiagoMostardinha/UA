...
volatile int voltMin =100;
volatile int voltMax = -1;
...
void _int_(32) isr_uart2(void)
{
    ...
    if (c == 'M')
    {
        // Send voltMax to the serial port UART2 using putstr
        char v1 = ('0' + ((voltMax| (16 | 3 << 16)) & 0x2)>>1);
        char v2 = ('0' + (voltMax| (16 | 3 << 16)) & 0x1);
        putstr("voltMax = ");
        putc(v1);      // Send "voltMin" to the serial port UART2
        putc(v2);      // Send "voltMin" to the serial port UART2
    }
    else if (c == 'm')
    {
        char v1 = '0' + ((voltMin| (16 | 3 << 16)) & 0x2)>>1;
        char v2 = '0' + (voltMin| (16 | 3 << 16)) & 0x1;;
        putstr("voltMin = ");
        putc(v1);      // Send "voltMin" to the serial port UART2
        putc(v2);      // Send "voltMin" to the serial port UART2
    }
...
int main(void)
{
    ...
    IEC1bits.U2RXIE = 1;        // disable U2RXIE, disable U2TXIE (register IEC1)
    IEC1bits.U2TXIE = 0;
