...
main:   
        ...
        lw      $t1,TRISB($s0)          # configurar RB1 como entrada
        ori     $t1,$t1,0x0001
        sw      $t1,TRISB($s0)
        
        li      $s1,0x0000              # count = 0
...
loop:                                   # while(1){
        ...
        andi    $t2,$t2,0x0001          #   $t2 = PORTB1

if:     beq     $t2,$0,else             #   if(PORTB1 == 1){    
        sll     $s1,$s1,1               #       $s1 = $s1 << 1
        bne     $s1,0x0000,end_if
        ori     $s1,$s1,0x0001
        j       end_if

else:   srl     $s1,$s1,1               #       $t2 = $t2 >> 1
        bne     $s1,0x0000,end_if
        ori     $s1,$s1,0x1000
end_if:
        andi    $s1,$s1,0x000F
