...
void init_pfifo(PriorityFIFO* pfifo)
{
   ...
   pthread_mutex_init(&pfifo->mutex, NULL);        // initialize the mutex
   pthread_cond_init(&pfifo->notEmpty, NULL);      // initialize the condition variable notEmpty
   pthread_cond_init(&pfifo->notFull,NULL);        // initialize the condition variable notFull
}
...
void insert_pfifo(PriorityFIFO* pfifo, uint32_t id, uint32_t priority)
{
   ...
   pthread_mutex_lock(&pfifo->mutex);                    // lock the mutex
   while(full_pfifo(pfifo)) {                            
      pthread_cond_wait(&pfifo->notFull, &pfifo->mutex); // wait until the fifo is not full
   }
   ...
   pthread_cond_broadcast(&pfifo->notEmpty);           // broadcast the condition variable notEmpty
   pthread_mutex_unlock(&pfifo->mutex);                // unlock the mutex
}
...
uint32_t retrieve_pfifo(PriorityFIFO* pfifo)
{
   ...
   while(empty_pfifo(pfifo)) {                           
      pthread_cond_wait(&pfifo->notEmpty, &pfifo->mutex);   // wait until the fifo is not empty
   }
   ...
   pthread_cond_broadcast(&pfifo->notFull);           // broadcast the condition variable notFull
   pthread_mutex_unlock(&pfifo->mutex);               // unlock the mutex
