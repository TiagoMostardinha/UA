...
typedef struct
{
   ...
   pthread_mutex_t mutex;        // mutex for patient
   pthread_cond_t treated;       // condition variable for patient to wait for treatment
} Patient;
...
void init_simulation(uint32_t np,uint32_t nnurses, uint32_t ndoctors)
{
   ...
   /* start nurses thread and life cycle */
   ...
   /* start doctors thread and life cycle */
   ...
   uint32_t patients_id[np];
   for (uint32_t i = 0; i < np; i++) {
      patients_id[i] = i;
      pthread_create(&patients[i], NULL, patient_life, &patients_id[i]);
   }
...
void patient_goto_urgency(int id)
{
   ...
   printf("\e[31;01mPatient %s (number %u): get to hospital\e[0m\n", hd->all_patients[id].name, id);
...
void patient_wait_end_of_consultation(int id)
{
   ...
   printf("\e[31;01mPatient %s (number %u): health problems treated\e[0m\n", hd->all_patients[id].name, id);
}

/* changes are required to this function */
void* patient_life(void* arg)
{
   uint32_t id = *((uint32_t*)arg);             // get patient id from arg
   ...
   pthread_exit(0);                             // patient finished
}
